#!/usr/bin/env bash
# US-052 Restart Validation: Cutoff Embedding Mode
#
...
# Tolerance: 1e-6 Ha (ThDD:Eq. T-US-052-V-1.1)
#
# Prerequisites:
...
#   GMX=/path/to/gmx bash tests/restart/run_restart_cutoff.sh
#
# LGPL-3.0-or-later
...
NSTEPS_TOTAL=40
NSTEPS_SEG1=20
RESTART_STEP=21  # First post-restart step (0-indexed step 20 is last of segment 1)

# Source files
MDP_SRC="${REPO_ROOT}/examples/b5_tutorial/nve_qmmm.mdp"
...
ARCHIVE_DIR="${REPO_ROOT}/tests/data/b5_restart/cutoff"

echo "========================================================================"
echo "US-052 Restart Validation: CUTOFF Embedding Mode"
echo "========================================================================"
echo "GMX binary: ${GMX}"
...
WORKDIR="$(mktemp -d /tmp/us052_cutoff_XXXXXX)"
...
# Copy base MDP and override critical parameters
cp "${MDP_SRC}" "${TEST_MDP}"
...
RESULTS_JSON="${WORKDIR}/results_cutoff.json"
cd "${REPO_ROOT}"
...
if [ ${COMPARE_EXIT} -eq 0 ] && [ "${SCC_FAILURES}" -eq 0 ]; then
    echo "US-052 CUTOFF RESTART VALIDATION: PASS"
    echo "  - QM energy match: PASS (within 1e-6 Ha)"
    echo "  - SCC convergence: PASS (0 failures)"
else
    echo "US-052 CUTOFF RESTART VALIDATION: FAIL"
